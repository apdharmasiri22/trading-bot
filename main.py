...
# State Management for Telegram Spam Protection
...
# Telegram Alert Function with Memory
def send_telegram_alert(token, chat_id, message, coin):
    if token and chat_id:
        state = load_state()
        if coin in state.get("last_alerts", []):
            return 
        url = f"https://api.telegram.org/bot{token}/sendMessage"
        try: 
            requests.post(url, json={"chat_id": chat_id, "text": message, "parse_mode": "Markdown"})
            state["last_alerts"].append(coin)
            save_state(state)
        except: pass
...
def analyze_coin_for_scanner(coin, htf, ltf):
    ...
    if bull_per >= 55 and htf_trend == "BULLISH":
        return {"Coin": coin_display, "Ticker": coin, "Signal": "🟩 BUY / LONG", "Strength": f"{bull_per:.1f}%", "Entry": f"${c_price:.{dec}f}", "SL": f"${c_price - (c_atr*2):.{dec}f}", "TP": f"${c_price + (c_atr*4):.{dec}f}"}
    elif bear_per >= 55 and htf_trend == "BEARISH":
        return {"Coin": coin_display, "Ticker": coin, "Signal": "🟥 SELL / SHORT", "Strength": f"{bear_per:.1f}%", "Entry": f"${c_price:.{dec}f}", "SL": f"${c_price + (c_atr*2):.{dec}f}", "TP": f"${c_price - (c_atr*4):.{dec}f}"}
    return None
    ...
    for r in list(results):
        if r is not None: 
            ...
            if tg_on and tg_token and tg_id:
                ...
                send_telegram_alert(tg_token, tg_id, msg, r['Ticker'])

if active_signals:
    st.dataframe(pd.DataFrame(active_signals).drop(columns=['Ticker']), use_container_width=True, hide_index=True)
else:
...
with col_chart:
    ...
    new TradingView.widget({{
      "width": "100%", "height": 450, "symbol": "BINANCE:{selected_coin}",
      "interval": "{tv_interval}", "timezone": "Etc/UTC", "theme": "dark",
...
if df_htf is not None and df_ltf is not None and df_1m is not None:
    ...
    with col_sig:
        if bull_per >= 55 and "BULLISH" in htf_trend:
        ...
        elif bear_per >= 55 and "BEARISH" in htf_trend:
